...
Every team places the ball in the same scenarios, and there are 2 tries per
scenario.
...
The points will be counted one time per scenario. For example, if the robot touches the ball both tries, only 1 point will be given for the team in this scenario.

The partial points of each scenario will be summed. In case of ties, the team that was capable of placing the balls in the least amount of time wins.
